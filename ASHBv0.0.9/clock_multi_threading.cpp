...
//Fait :)
    ...
    //Multi threading
    void colli_presence_cl(){
        std::cout << "colli_presence_cl: thread2" << std::endl;
        collision collision;
        model mod_obj;
        Data obj;
        std::ifstream file("./data/CharacterData.csv");
        ...
        while (getline(file, line)) {
            ...
            if (is_sick(row[0])) {
                ...
                obj.health(row[0], -num_generator(1, 3) - obj.fastdiv(stoi(obj.get_value_char(row[0], 5)), 13.5));
            ...
            obj.desire(row[0], num_generator(3,8), false);
            death(row[0]);
    ...
    void stats_cl(){
        std::cout << "stats_cl: thread3" << std::endl;
        Data obj; 
        std::ifstream file("./data/CharacterData.csv");
        std::string line;
        collision collision;
        ...
        while (getline(file, line)) {
            ...
            std::cout << row[0] << std::endl;
             int hap_const = num_generator(3,8);
            ...
            if (obj.get_value_char(row[0], 13) == "no") {
                ...
                if (obj.point(row[0]) != "not") {
                    ...
                    hap_const = -num_generator(6, 12) - (couples_list.size());
                } else {
                    if (obj.procreation(row[0])) {
                        hap_const = num_generator(23, 37);
            ...
            if (is_sick(row[0])) {
                if (stoi(obj.get_value_char(row[0], 2)) <= 65) {
                    ...
                    obj.bonheur(row[0], -num_generator(3, 6) - obj.fastdiv(stoi(obj.get_value_char(row[0], 3)), 1.0 * num_generator(8, 12)) + hap_const);
                } else {                
                    //obj.bonheur(row[0], -num_generator(3, 4) - stoi(obj.get_value_char(row[0], 3)) / num_generator(9, 12) + hap_const);
                    obj.bonheur(row[0], -num_generator(2, 4) - obj.fastdiv(stoi(obj.get_value_char(row[0], 3)),1.0 * num_generator(8, 12)) + hap_const);
                }
            } else {
    ...
    void movement_cl(const std::string& id){
        std::cout << "movement_cl: thread1" << std::endl;
...
// 
    void main_loop() {    
        ...
        while (getline(file, line)) {
            ...
            if (is_sick(row[0])) {
                ...
                //obj.health(row[0], -num_generator(1, 3) - (99 - stoi(obj.get_value_char(row[0], 5))) / 19);
                obj.health(row[0], -num_generator(1, 3) - obj.fastdiv(stoi(obj.get_value_char(row[0], 5)), 17.0));
                if (stoi(obj.get_value_char(row[0], 3)) <= 40) {
                    //obj.stress(row[0], num_generator(4, 8) + stoi(obj.get_value_char(row[0], 3)) / 8);
                    obj.stress(row[0], num_generator(4, 8) + obj.fastdiv(stoi(obj.get_value_char(row[0], 3)), 6.0));
                } else {
            ...
            if (stoi(obj.get_value_char(row[0], 10)) <= 30) {
                ...
                obj.health(row[0], num_generator(7, 9));
            }
    ...
    int main() {    
        ...
        std::thread thread1(movement_cl);
        std::thread thread2(colli_presence_cl);
        std::thread thread3(stats_cl);
