...
public:
    movement(); 
    void single_char_movement(const std::string& id, model& model_obj, Data& Data_obj);
    std::string get_simulation_param(int line);    
    std::string get_majority_direction(const std::string& id, model& model_obj, Data& Data_obj);
